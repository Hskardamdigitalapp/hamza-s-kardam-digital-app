...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _pickPhoto() async {
    ...
    try { await WalletService.uploadAvatar(await picked.readAsBytes()); if (mounted) setState(() {}); }
  ...
  Future<void> _verify(int tier) async {
    ...
    final result = await showDialog<bool>(context: context, builder: (ctx) => StatefulBuilder(builder: (ctx, setDialog) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        const Text('Choose one verification method. Your verification is submitted for review; it is not automatically approved.'),
        ...
        TextField(controller: ref, keyboardType: TextInputType.number, obscureText: true, decoration: InputDecoration(labelText: '$method number', prefixIcon: const Icon(Icons.verified_user_outlined), helperText: 'Do not share this number in chat.'),),
      ]),
      actions: [TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancel')), ElevatedButton(onPressed: () async { if (ref.text.trim().length < 4) return; Navigator.pop(ctx, true); }, child: const Text('Submit'))],
    )));
    if (result != true) { ref.dispose(); return; }
    try {
      await WalletService.requestKycUpgrade(method: method, reference: ref.text.trim(), targetTier: tier);
      if (mounted) { setState(_load); _snack('Verification submitted. Your status is now pending review.'); }
    ...
    return Scaffold(backgroundColor: _bg, appBar: AppBar(title: const Text('Profile'), backgroundColor: _bg, foregroundColor: Colors.white, elevation: 0),
      body: FutureBuilder<Map<String, dynamic>?>(future: _profileFuture, builder: (context, ps) {
        ...
        return FutureBuilder<Map<String, dynamic>>(future: _kycFuture, builder: (context, ks) {
          ...
          return RefreshIndicator(color: _gold, onRefresh: () async => setState(_load), child: ListView(padding: const EdgeInsets.fromLTRB(18, 8, 18, 100), children: [
            ...
            const SizedBox(height: 10), Center(child: Text(name, style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w900))), Center(child: Text(phone.isNotEmpty ? phone : email, style: const TextStyle(color: _muted))),
            ...
            _item(Icons.dialpad_outlined, 'Transaction PIN', 'Change your transaction PIN', () => _snack('Transaction PIN setup will be connected to secure wallet authorization.')),
            _item(Icons.fingerprint, 'Biometrics', 'Register your fingerprint / face ID', () => _snack('Biometric security will use your device authentication.')),
            _item(Icons.lock_reset, 'Change Password', 'Update your login password', () => _snack('Use the password reset flow from the login screen.')),
            _item(Icons.card_giftcard, 'Refer & Earn', 'Invite friends and earn rewards', () => _snack('Referral rewards are coming with the rewards backend.')),
            ...
            const SizedBox(height: 16), FutureBuilder<bool>(future: _adminFuture, builder: (context, a) => a.data == true ? _item(Icons.admin_panel_settings_outlined, 'Admin Dashboard', 'Manage users and service activity', () => Navigator.pushNamed(context, '/admin')) : const SizedBox.shrink()),
  ...
  Future<void> _confirmDelete() async { final ok = await showDialog<bool>(context: context, builder: (c) => AlertDialog(title: const Text('Delete Account?'), content: const Text('This action should only be used when the account deletion process is fully configured.'), actions: [TextButton(onPressed: () => Navigator.pop(c, false), child: const Text('Cancel')), TextButton(onPressed: () => Navigator.pop(c, true), child: const Text('Continue'))])); if (ok == true && mounted) _snack('Account deletion is not enabled yet, so your account remains safe.'); }
}
