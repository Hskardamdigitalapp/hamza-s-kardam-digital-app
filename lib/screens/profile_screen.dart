...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void _load() {
    ...
    _kycFuture = WalletService.getMyKycStatus();
  }
  ...
  Future<void> _chooseProfileImage() async {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Profile Picture', style: TextStyle(color: _navy, fontSize: 20, fontWeight: FontWeight.w900)),
              const SizedBox(height: 12),
              ListTile(
                leading: const CircleAvatar(backgroundColor: _goldLight, child: Icon(Icons.person, color: _navy)),
                title: const Text('Use Avatar', style: TextStyle(fontWeight: FontWeight.w800)),
                subtitle: const Text('Use your account avatar instead of a real photo'),
                onTap: () => Navigator.pop(ctx, 'avatar'),
              ),
              ListTile(
                leading: const CircleAvatar(backgroundColor: _goldLight, child: Icon(Icons.photo_camera_outlined, color: _navy)),
                title: const Text('Use Real Picture', style: TextStyle(fontWeight: FontWeight.w800)),
                subtitle: const Text('Choose your own picture from the gallery'),
    ...
    if (choice == 'avatar') {
      try {
        await WalletService.useDefaultAvatar();
    ...
    try {
      await WalletService.uploadAvatar(await picked.readAsBytes());
      if (mounted) setState(_load);
    } catch (e) {
  ...
  Future<void> _verify(int tier) async {
          ...
          content: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                onChanged: (v) => setDialog(() => method = v ?? 'NIN'),
              ),
              const SizedBox(height: 12),
              TextField(
                controller: ref,
                ...
                decoration: InputDecoration(
                  ...
                  prefixIcon: const Icon(Icons.verified_user_outlined, color: _navy),
                  suffixIcon: IconButton(icon: const Icon(Icons.clear), onPressed: ref.clear),
                  counterText: '',
    ...
    try {
      ...
      if (mounted) {
        setState(_load);
        _snack('Identity verification submitted securely.');
      }
    } catch (e) {
      if (mounted) _snack(e.toString().replaceFirst('Exception: ', ''));
    } finally {
  ...
  Future<void> _confirmDelete() async {
    ...
    try {
      ...
      if (mounted) _snack('Unable to check wallet balance. Try again.');
      return;
    }
      ...
      await showDialog<void>(
        ...
        builder: (c) => AlertDialog(
          ...
          title: const Row(children: [
            ...
            SizedBox(width: 8),
            Expanded(child: Text('Account Cannot Be Deleted')),
          ]),
          content: Text('You still have ₦${balance.toStringAsFixed(2)} in your wallet. Please withdraw or spend the remaining money until your wallet balance is ₦0.00.'),
          actions: [
            TextButton(onPressed: () => Navigator.pop(c), child: const Text('Close')),
            ElevatedButton(
              style: ElevatedButton.styleFrom(backgroundColor: _navy, foregroundColor: Colors.white),
              onPressed: () { Navigator.pop(c); Navigator.pushNamed(context, '/wallet'); },
    ...
    try {
      await WalletService.deleteAccount();
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: FutureBuilder<Map<String, dynamic>?>(
          ...
          builder: (context, snap) {
            final p = snap.data ?? {};
            final name = p['full_name']?.toString().trim().isNotEmpty == true ? p['full_name'].toString().trim() : 'Customer';
            return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('${_greeting()}, $name', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
              const Text('Profile', style: TextStyle(fontSize: 12, color: Colors.white70)),
            ]);
          },
        ),
        actions: [
          IconButton(tooltip: 'Notifications', onPressed: () => _snack('No new notifications.'), icon: const Icon(Icons.notifications_none_rounded, color: _goldLight)),
        ],
      ),
      body: FutureBuilder<Map<String, dynamic>?>(
        ...
        builder: (context, ps) {
          final p = ps.data ?? {};
          final name = p['full_name']?.toString().trim().isNotEmpty == true ? p['full_name'].toString().trim() : 'Customer';
          final email = p['email']?.toString().trim().isNotEmpty == true ? p['email'].toString().trim() : (user?.email ?? '');
      ...
      bottomNavigationBar: NavigationBar(
        ...
        destinations: const [
          ...
          NavigationDestination(icon: Icon(Icons.receipt_long_outlined, color: _navy), label: 'Orders'),
          ...
          NavigationDestination(icon: Icon(Icons.person_outline, color: _gold), selectedIcon: Icon(Icons.person, color: _navy), label: 'Profile'),
        ],
      ),
    );
  }
  ...
  Widget _profileHero(String name, String email, String? avatar) => Container(
    ...
    child: Column(children: [
      Stack(children: [
        Container(
          ...
          child: CircleAvatar(
            ...
            child: avatar?.isNotEmpty == true ? null : const Icon(Icons.person, size: 48, color: _navy),
          ),
        ),
        Positioned(
          right: 0,
          ...
          child: Material(
            ...
            child: InkWell(
              onTap: _uploading ? null : _chooseProfileImage,
              customBorder: const CircleBorder(),
              child: Padding(padding: const EdgeInsets.all(9), child: Icon(_uploading ? Icons.hourglass_top : Icons.camera_alt, size: 18, color: _navy)),
            ),
          ),
        ),
      ]),
      ...
      Container(
        ...
        decoration: BoxDecoration(color: _gold.withOpacity(.18), borderRadius: BorderRadius.circular(20), border: Border.all(color: _gold.withOpacity(.55))),
        child: const Text('HAMZA S. KARDAM DIGITAL APP', style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: .8)),
      ),
    ]),
  );

  Widget _limitsCard(int tier) {
    final wallet = tier >= 3 ? '₦5,000,000' : tier >= 2 ? '₦500,000' : '₦50,000';
    final today = tier >= 3 ? '₦3,000,000' : tier >= 2 ? '₦100,000' : '₦20,000';
    return Container(
      padding: const EdgeInsets.all(18),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(22), border: Border.all(color: _gold.withOpacity(.18))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [const Icon(Icons.account_balance_wallet, color: _gold), const SizedBox(width: 10), const Text('Wallet & Limits', style: TextStyle(color: _navy, fontSize: 18, fontWeight: FontWeight.w900)), const Spacer(), _badge('Tier $tier')]),
        const SizedBox(height: 16),
        _limit('Wallet limit', wallet),
        _limit('Today limit', today),
      ]),
    );
  }

  Widget _tierUpgradeCard(int tier, String status) {
    if (tier >= 3) return _item(Icons.verified, 'Tier 3 verified', 'Your account has the highest available tier', () => _snack('Your account is already Tier 3.'));
    final next = tier + 1;
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Row(children: [Icon(Icons.workspace_premium, color: _gold), SizedBox(width: 10), Text('TIER UPGRADE', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900))]),
        const SizedBox(height: 10),
        Text('Upgrade to Tier $next for higher wallet and transaction limits.', style: const TextStyle(color: Colors.white70, height: 1.35)),
        const SizedBox(height: 14),
        SizedBox(width: double.infinity, child: ElevatedButton(onPressed: pending ? null : () => _verify(next), style: ElevatedButton.styleFrom(backgroundColor: _gold, foregroundColor: _navy), child: Text(pending ? 'Verification Pending' : 'Verify & Upgrade'))),
      ]),
    );
  }

  Widget _item(IconData icon, String title, String subtitle, VoidCallback onTap, {bool danger = false}) => Card(
    elevation: 0,
    color: Colors.white,
    ...
    child: ListTile(
      onTap: onTap,
      leading: CircleAvatar(backgroundColor: danger ? _danger.withOpacity(.1) : _goldLight, child: Icon(icon, color: danger ? _danger : _navy)),
      title: Text(title, style: TextStyle(color: danger ? _danger : _navy, fontWeight: FontWeight.w900)),
      subtitle: Text(subtitle, style: const TextStyle(color: _muted)),
      trailing: Icon(Icons.chevron_right, color: danger ? _danger : _navy),
    ),
  );

  Widget _badge(String text) => Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5), decoration: BoxDecoration(color: _goldLight, borderRadius: BorderRadius.circular(20)), child: Text(text, style: const TextStyle(color: _navy, fontWeight: FontWeight.w900, fontSize: 12)));
