...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: _bg,
      appBar: AppBar(title: const Text('Profile', style: TextStyle(fontWeight: FontWeight.w900)), backgroundColor: _navy, foregroundColor: Colors.white, elevation: 0),
      body: FutureBuilder<Map<String, dynamic>?>(future: _profileFuture, builder: (context, ps) {
        ...
        return FutureBuilder<Map<String, dynamic>>(future: _kycFuture, builder: (context, ks) {
          ...
          return RefreshIndicator(color: _gold, onRefresh: () async { setState(_load); await _kycFuture; }, child: ListView(padding: const EdgeInsets.fromLTRB(16, 16, 16, 110), children: [
            _profileHero(name, phone.isNotEmpty ? phone : email, avatar),
            ...
            const SizedBox(height: 12),
            SizedBox(width: double.infinity, height: 52, child: OutlinedButton.icon(onPressed: () async { await WalletService.logout(); if (mounted) Navigator.pushNamedAndRemoveUntil(context, '/login', (_) => false); }, icon: const Icon(Icons.logout), label: const Text('Logout', style: TextStyle(fontWeight: FontWeight.w900)), style: OutlinedButton.styleFrom(foregroundColor: _navy, side: const BorderSide(color: _gold, width: 1.5), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))))),
          ]));
        });
      }),
  ...
  Widget _profileHero(String name, String contact, String? avatar) => Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(gradient: const LinearGradient(colors: [_navy, _navy2], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(26), boxShadow: [BoxShadow(color: _navy.withOpacity(.18), blurRadius: 18, offset: const Offset(0, 8))]), child: Column(children: [Stack(children: [Container(padding: const EdgeInsets.all(3), decoration: const BoxDecoration(color: _gold, shape: BoxShape.circle), child: CircleAvatar(radius: 45, backgroundColor: Colors.white, backgroundImage: avatar?.isNotEmpty == true ? NetworkImage(avatar!) : null, child: avatar?.isNotEmpty == true ? null : const Icon(Icons.person, size: 48, color: _navy))), Positioned(right: 0, bottom: 0, child: Material(color: _gold, shape: const CircleBorder(), child: InkWell(onTap: _uploading ? null : _pickPhoto, customBorder: const CircleBorder(), child: Padding(padding: const EdgeInsets.all(9), child: Icon(_uploading ? Icons.hourglass_top : Icons.camera_alt, size: 18, color: _navy)))))]), const SizedBox(height: 12), Text(name, style: const TextStyle(color: Colors.white, fontSize: 23, fontWeight: FontWeight.w900)), const SizedBox(height: 4), Text(contact, style: TextStyle(color: Colors.white.withOpacity(.72), fontSize: 14)), const SizedBox(height: 12), Container(padding: const EdgeInsets.symmetric(horizontal: 13, vertical: 7), decoration: BoxDecoration(color: _gold.withOpacity(.18), borderRadius: BorderRadius.circular(20), border: Border.all(color: _gold.withOpacity(.55))), child: const Text('HAMZA S. KARDAM DIGITAL APP', style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: .8)))]));
  ...
  Future<void> _confirmDelete() async { await showDialog<void>(context: context, builder: (c) => AlertDialog(title: Row(children: [const Icon(Icons.warning_amber_rounded, color: _danger), const SizedBox(width: 8), const Expanded(child: Text('Delete Account?'))]), content: const Text('This action is permanent. Secure account deletion is not available yet.'), actions: [TextButton(onPressed: () => Navigator.pop(c), child: const Text('Close'))])); }
}
