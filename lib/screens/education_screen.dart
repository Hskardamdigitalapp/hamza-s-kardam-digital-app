  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.fromLTRB(28, 18, 28, 40), children: [
        Container(padding: const EdgeInsets.fromLTRB(36, 28, 22, 28), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF211F16), Color(0xFF171717)]), borderRadius: BorderRadius.circular(28), border: Border.all(color: _gold.withValues(alpha: .65))), child: Row(children: [const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Education', style: TextStyle(color: Colors.white, fontSize: 29, fontWeight: FontWeight.w900)), SizedBox(height: 8), Text('Buy result pins & JAMB profile pins instantly.', style: TextStyle(color: Colors.white70, fontSize: 17, height: 1.35, fontWeight: FontWeight.w600))])), Container(width: 92, height: 92, decoration: const BoxDecoration(color: _gold, shape: BoxShape.circle), child: const Icon(Icons.school_rounded, color: Color(0xFF151515), size: 48))])),
        const SizedBox(height: 22),
        Container(padding: const EdgeInsets.all(22), decoration: BoxDecoration(color: const Color(0xFF1A1A1D), borderRadius: BorderRadius.circular(24)), child: Row(children: [Container(width: 52, height: 52, decoration: BoxDecoration(color: _gold.withValues(alpha: .12), borderRadius: BorderRadius.circular(15)), child: const Icon(Icons.tag, color: _gold, size: 30)), const SizedBox(width: 14), const Expanded(child: Text('Quantity', style: TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.w900)),), Text('Max 5 per purchase', style: const TextStyle(color: Colors.white54, fontWeight: FontWeight.w700)), const SizedBox(width: 8), IconButton(onPressed: _quantity <= 1 ? null : () => setState(() => _quantity--), icon: const Icon(Icons.remove_circle_outline, color: Colors.white54)), Text('$_quantity', style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900)), IconButton(onPressed: _quantity >= 5 ? null : () => setState(() => _quantity++), icon: const Icon(Icons.add_circle_outline, color: _gold))])),
