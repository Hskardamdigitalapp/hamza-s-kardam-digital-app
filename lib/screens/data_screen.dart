...
class _DataScreenState extends State<DataScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(status == 'delivered' ? 'Data delivered successfully.' : status == 'processing' ? 'Data order is processing. Check Transactions for updates.' : 'Data order failed.')));
      if (status == 'delivered' || status == 'processing') _phone.clear();
    } catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Unable to buy data: $e'))); }
    finally { if (mounted) setState(() => _buying = false); }
  }

  void _pickBeneficiary(String number) { setState(() => _phone.text = number); }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: _bg,
      appBar: AppBar(backgroundColor: _navy, foregroundColor: Colors.white, title: const Text('Buy Data', style: TextStyle(fontWeight: FontWeight.w900)), actions: [IconButton(tooltip: 'History', onPressed: () => Navigator.pushNamed(context, '/transactions'), icon: const Icon(Icons.history_rounded))]),
      body: Form(key: _formKey, child: ListView(padding: const EdgeInsets.fromLTRB(16, 18, 16, 30), children: [
        ...
        const SizedBox(height: 9),
        if (_loadingPlans) const Center(child: Padding(padding: EdgeInsets.all(24), child: CircularProgressIndicator())) else if (_plans.isEmpty) const Text('No plans available. Check provider configuration.', style: TextStyle(color: Colors.red)) else _planList(),
