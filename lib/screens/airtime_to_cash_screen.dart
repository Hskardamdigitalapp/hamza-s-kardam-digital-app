...
class _AirtimeToCashScreenState extends State<AirtimeToCashScreen> {
  ...
  final _phone = TextEditingController(), _amount = TextEditingController(), _bank = TextEditingController(), _account = TextEditingController(), _name = TextEditingController();
  String _network = 'MTN';
  bool _loading = false;
  ...
  @override Widget build(BuildContext context) => Scaffold(
    ...
    body: FutureBuilder<Map<String, dynamic>>(future: _kycFuture, builder: (context, snap) {
      ...
      return Form(key: _formKey, child: ListView(padding: const EdgeInsets.all(20), children: [
        ...
        const SizedBox(height: 14),
        TextFormField(controller: _amount, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Airtime amount', prefixText: '₦ ', prefixIcon: Icon(Icons.payments_outlined)), validator: (v) { final amount = double.tryParse(v?.trim() ?? ''); return amount == null || amount < 100 ? 'Minimum amount is ₦100' : null; }),
  ...
  Widget _locked(int tier, String status) {
    ...
    return Center(child: SingleChildScrollView(padding: const EdgeInsets.all(22), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Container(width: 82, height: 82, decoration: const BoxDecoration(color: Color(0xFFF4E8C6), shape: BoxShape.circle), child: const Icon(Icons.lock_outline, size: 42, color: Color(0xFF061B49))),
      const SizedBox(height: 20), const Text('Tier 2 Verification Required', textAlign: TextAlign.center, style: TextStyle(fontSize: 25, fontWeight: FontWeight.w900, color: Color(0xFF061B49))),
      const SizedBox(height: 10), Text(pending ? 'Your NIN/BVN verification is pending review.' : 'Airtime to Cash is available only after your account reaches verified Tier 2.', textAlign: TextAlign.center, style: const TextStyle(color: Colors.black54, fontSize: 15)),
      const SizedBox(height: 20), Text('Current account: Tier $tier', style: const TextStyle(fontWeight: FontWeight.w800)), const SizedBox(height: 16),
