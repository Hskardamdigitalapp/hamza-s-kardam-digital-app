...
class _AirtimeScreenState extends State<AirtimeScreen> {
  static const _navy = Color(0xFF061B49), _gold = Color(0xFFC89B3C), _bg = Color(0xFFF5F7FB);
  static const _storage = FlutterSecureStorage();
  final _formKey = GlobalKey<FormState>();
  final _phone = TextEditingController(), _amount = TextEditingController();
  String _network = 'MTN';
  bool _loading = false, _saveBeneficiary = false;
  List<String> _beneficiaries = [];

  @override void initState() { super.initState(); _loadBeneficiaries(); }
  ...
  @override Widget build(BuildContext context) => Scaffold(
    ...
    body: Form(key: _formKey, child: ListView(padding: const EdgeInsets.fromLTRB(16, 18, 16, 30), children: [
      ...
      const SizedBox(height: 5), const Text('Buy Airtime', style: TextStyle(color: _navy, fontSize: 27, fontWeight: FontWeight.w900)),
      const SizedBox(height: 4), const Text('Top up any Nigerian line instantly for you or a friend.', style: TextStyle(color: Colors.black54)),
      const SizedBox(height: 20), _sectionLabel('Phone Number'),
      ...
      const SizedBox(height: 7), _sectionLabel('SELECT NETWORK'), const SizedBox(height: 9), _networkSelector(),
      const SizedBox(height: 20), _sectionLabel('QUICK AMOUNT'), const SizedBox(height: 9),
      ...
      const SizedBox(height: 20), _sectionLabel('OR ENTER AMOUNT'), const SizedBox(height: 8),
      ...
      const SizedBox(height: 8), CheckboxListTile(value: _saveBeneficiary, onChanged: (v) => setState(() => _saveBeneficiary = v ?? false), contentPadding: EdgeInsets.zero, activeColor: _gold, title: const Text('Save as Beneficiary', style: TextStyle(fontWeight: FontWeight.w800)), subtitle: const Text('Quickly reuse this number next time.')),
      const SizedBox(height: 8), _ussdCard(), const SizedBox(height: 18),
