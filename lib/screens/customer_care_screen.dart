...
class CustomerCareScreen extends StatelessWidget {
  ...
  Future<void> _whatsapp(BuildContext context) async {
    final ok = await launchUrl(Uri.parse('https://wa.me/2349044444921?text=Hello%20HAMZA%20S.%20KARDAM%20DIGITAL%20APP%2C%20I%20need%20customer%20care%20support.'), mode: LaunchMode.externalApplication);
  ...
  Future<void> _group(BuildContext context) async {
    final ok = await launchUrl(Uri.parse(_whatsappGroup), mode: LaunchMode.externalApplication);
    if (!ok && context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('WhatsApp Group could not be opened.')));
  }
    ...
    showModalBottomSheet<void>(
      ...
      builder: (ctx) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(color: dark ? _dark : const Color(0xFFF5F7FB), borderRadius: const BorderRadius.vertical(top: Radius.circular(30))),
          child: SafeArea(
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const Row(children: [CircleAvatar(backgroundColor: Color(0x26B8860B), child: Icon(Icons.auto_awesome_rounded, color: _gold)), SizedBox(width: 12), Text('AI Assistant', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900))]),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: _navy, foregroundColor: Colors.white, title: const Text('Customer Support', style: TextStyle(fontWeight: FontWeight.w900))),
      body: SafeArea(
        child: ListView(padding: const EdgeInsets.fromLTRB(18, 18, 18, 30), children: [
          Container(padding: const EdgeInsets.fromLTRB(22, 24, 22, 24), decoration: const BoxDecoration(gradient: LinearGradient(colors: [_navy, _navy2]), borderRadius: BorderRadius.all(Radius.circular(26))), child: const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [CircleAvatar(radius: 25, backgroundColor: Color(0x26B8860B), child: Icon(Icons.support_agent_rounded, color: _gold, size: 30)), SizedBox(width: 14), Text('HAMZA S. KARDAM', style: TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w900, letterSpacing: 1.2))]), SizedBox(height: 22), Text('How can we help?', style: TextStyle(color: Colors.white, fontSize: 28, fontWeight: FontWeight.w900)), SizedBox(height: 6), Text('Our support team is available to assist you.', style: TextStyle(color: Colors.white70, fontSize: 16, fontWeight: FontWeight.w600)), SizedBox(height: 16), Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.circle, color: Color(0xFF39D98A), size: 12), SizedBox(width: 8), Text('Online · Replies in mins', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700))])])),
          ...
          Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(color: dark ? const Color(0xFF1E2025) : Colors.white, borderRadius: BorderRadius.circular(22), border: Border.all(color: _gold.withValues(alpha: .18))), child: Column(children: [
            _contact(context, Icons.call_outlined, 'Phone Number', '0707 777 7636', () => _call(context)),
            const Divider(height: 24),
            _contact(context, Icons.chat_outlined, 'WhatsApp', '09044444921', () => _whatsapp(context), whatsapp: true),
            const Divider(height: 24),
            _contact(context, Icons.groups_rounded, 'WhatsApp Group', 'Join our community group', () => _group(context), whatsapp: true),
          ])),
          const SizedBox(height: 20),
          const Text('Recent help', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
          const SizedBox(height: 10),
          Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: dark ? const Color(0xFF1E2025) : Colors.white, borderRadius: BorderRadius.circular(20)), child: const Row(children: [CircleAvatar(backgroundColor: Color(0x26B8860B), child: Icon(Icons.headset_mic_outlined, color: _gold)), SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('HAMZA S. KARDAM DIGITAL APP Customer Care', style: TextStyle(fontWeight: FontWeight.w900)), SizedBox(height: 4), Text('We are here to help with your digital services.', style: TextStyle(color: Colors.grey))]))])),
        ]),
      ),
    );
  }

  Widget _supportCard(BuildContext context, IconData icon, String title, String sub, VoidCallback onTap) {
    final dark = Theme.of(context).brightness == Brightness.dark;
    return Container(margin: const EdgeInsets.only(bottom: 12), decoration: BoxDecoration(color: dark ? const Color(0xFF1E2025) : Colors.white, borderRadius: BorderRadius.circular(22), border: Border.all(color: _gold.withValues(alpha: .18))), child: ListTile(contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 9), leading: Container(width: 56, height: 56, decoration: BoxDecoration(color: _gold.withValues(alpha: .16), borderRadius: BorderRadius.circular(18)), child: Icon(icon, color: _gold, size: 29)), title: Text(title, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900)), subtitle: Padding(padding: const EdgeInsets.only(top: 5), child: Text(sub)), trailing: Container(width: 42, height: 42, decoration: BoxDecoration(color: _gold, borderRadius: BorderRadius.circular(14)), child: const Icon(Icons.arrow_forward_rounded, color: _navy)), onTap: onTap));
  }

  Widget _contact(BuildContext context, IconData icon, String label, String value, VoidCallback onTap, {bool whatsapp = false}) {
    return InkWell(onTap: onTap, borderRadius: BorderRadius.circular(16), child: Row(children: [Container(width: 44, height: 44, decoration: BoxDecoration(color: _gold.withValues(alpha: .16), borderRadius: BorderRadius.circular(14)), child: whatsapp ? Image.network('https://img.icons8.com/color/48/whatsapp--v1.png', width: 25, height: 25, errorBuilder: (_, __, ___) => Icon(icon, color: _gold)) : Icon(icon, color: _gold)), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: const TextStyle(fontWeight: FontWeight.w800)), const SizedBox(height: 2), Text(value, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700))])), const Icon(Icons.chevron_right)]));
  }
}
